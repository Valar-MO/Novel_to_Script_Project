...
from backend.services.chapter_detector import (
    ChapterSpan,
)
...
class TestProjectStorage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = (
            self.test_root / "test.db"
        )

        self.projects_directory = (
            self.test_root / "projects"
        )
    ...
    def _build_valid_file_data(
        ...
        *,
        ...
        raw_content = original_text.encode(
            "utf-8"
        )
        ...
        first_chunk_text = processed_text[
            :second_chunk_start
        ]

        second_chunk_text = processed_text[
            second_chunk_start:
        ]
    ...
    def test_save_project_creates_files_and_database_rows(
        self,
    ):
        """保存项目后应创建文件目录和数据库记录。"""
    ...
    def test_get_project_summary_excludes_full_chunk_text(
        self,
    ):
        """项目摘要应返回预览，但不返回完整正文。"""
        ...
        self.assertEqual(
            stored_file["chapter_count"],
            1,
        )

        self.assertEqual(
            stored_file["chunk_count"],
            2,
        )
        ...
        self.assertEqual(
            ...
            file_data.chunks[0].text[
                :200
            ],
        )

    def test_get_project_chunk_returns_full_text(
        self,
    ):
        """单块详情查询应返回完整正文和来源信息。"""
    ...
    def test_invalid_chunk_data_is_not_saved(
        self,
    ):
        """无效文本块不应创建目录或项目记录。"""
    ...
    def test_database_failure_rolls_back_and_cleans_files(
        self,
    ):
        """数据库写入失败时应清理临时项目文件。"""
    ...
    def test_missing_project_raises_error(
        self,
    ):
        """查询不存在的项目应抛出明确异常。"""
    ...
    def test_missing_chunk_raises_error(
        self,
    ):
        """查询不存在的文本块应抛出明确异常。"""
