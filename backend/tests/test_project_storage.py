...
class TestProjectStorage(unittest.TestCase):
    ...
    def _build_valid_file_data(
        ...
        *,
        ...
        second_chunk_start = processed_text.index(
            "第二段内容。"
        )
        ...
        first_chunk = TextChunk(
            chunk_id=(
                f"chunk_{global_order_start:04d}"
            ),
        ...
        second_chunk = TextChunk(
            chunk_id=(
                f"chunk_{global_order_start + 1:04d}"
            ),
    ...
    def test_get_project_returns_nested_project_data(
        self,
    ):
        """项目查询应返回文件、章节和文本块。"""
        ...
        stored_chapter = (
            stored_file["chapters"][0]
        )

        self.assertEqual(
            stored_chapter["chapter_number"],
            1,
        )
        self.assertEqual(
            stored_chapter["chapter_title"],
            "测试",
        )
        self.assertTrue(
            stored_chapter["is_detected"]
        )
        self.assertEqual(
            stored_chapter["chunk_count"],
            2,
        )

        stored_chunks = stored_file["chunks"]
